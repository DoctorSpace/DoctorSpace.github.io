...
const ViewWindow = ({ visible, setVisible, data, update }) => {
  ...
  function changeCardInfo(e) {
    ...
    let newTitle =
      takeTitle.current.value !== takeTitle.current.placeholder &&
      takeTitle.current.value
        ? takeTitle.current.value
        : data.title;
    let newCategory =
      takeCategory.current.value !== takeCategory.current.placeholder &&
      takeCategory.current.value
        ? takeCategory.current.value
        : data.category;
    let newPrice =
      takePrice.current.value !== takePrice.current.placeholder &&
      takePrice.current.value
        ? takePrice.current.value
        : data.price;
    ...
    <div className={rootClasses.join(" ")} onClick={() => setVisible(false)}>
      <div className="ViewWindow__content" onClick={(e) => e.stopPropagation()}>
        ...
        <div
          id="dropFile"
          for="image_uploads"
          className="ViewWindow__content-img"
        >
          <img src={data.url_img} />
        </div>

        <button
          className="ViewWindow__content-buttonClose"
        ...
        <form>
          ...
          <button onClick={deleteCardInfo} id="btnDeleteCard">
            {" "}
            Удалить карточку{" "}
          </button>
          <button onClick={changeCardInfo} id="btnChangeCard">
            {" "}
            Изменить карточку{" "}
          </button>
        </form>
